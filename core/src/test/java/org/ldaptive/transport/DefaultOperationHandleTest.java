...
public class DefaultOperationHandleTest
{
  ...
  public Object[][] createHandlesTimeoutResult()
  {
    return
      new Object[][] {
        new Object[] {
   ...
   */
  @Test(groups = "transport", dataProvider = "handles")
  public void awaitResult(
    final DefaultOperationHandle<SearchRequest, SearchResponse> handle,
    ...
    throws Exception
  {
    ...
    if (throwsTimeout) {
      ...
      final SearchResponse r = handle.execute();
    ...
    throws Exception
  {
    ...
    executorService.schedule(() -> handle.result(response), responseTime.toMillis(), TimeUnit.MILLISECONDS);
    if (throwsTimeout &&
        handle.getResponseTimeoutCondition() == DefaultOperationHandle.DEFAULT_RESPONSE_TIMEOUT_CONDITION)
    {
      try {
        handle.execute();
        Assert.fail("Should have thrown exception");
      ...
      final SearchResponse r = handle.execute();
      Assert.assertEquals(r, SearchResponse.builder().messageID(1).entry(entry1, entry2).reference(ref1).build());
    }
  }
