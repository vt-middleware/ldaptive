...
public class RoundRobinConnectionStrategyTest
{
  ...
  /**
   * Unit test for {@link RoundRobinConnectionStrategy#next(LdapURLSet)}.
   *
   * @param  actual  to initialize strategy with
   * @param  expected  to compare
   * @param  count  number of times to invoke {@link LdapURLSet#doWithNextActiveUrl(Consumer)} before assertions
   *
   * @throws  LdapException  On LDAP operation errors.
   */
  @Test(groups = "conn", dataProvider = "urls")
  public void next(final String actual, final LdapURL[] expected, final int count) throws LdapException
  {
    final LdapURLSet urlSet = new LdapURLSet(new RoundRobinConnectionStrategy(), actual);
    for (int i = 0; i < count; i++) {
      urlSet.doWithNextActiveUrl(u -> {});
    }
    for (LdapURL url : expected) {
      urlSet.doWithNextActiveUrl(u -> Assert.assertEquals(url, u));
    }
  }
