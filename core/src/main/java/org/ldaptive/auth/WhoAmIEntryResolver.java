...
/**
 * Executes the {@link WhoAmIOperation} on the authenticated connection, then performs an object level search on the
 * result. Useful when users authenticate with some mapped identifier, like DIGEST-MD5.
 *
...
public class WhoAmIEntryResolver extends AbstractSearchEntryResolver
{
  ...
  {
    final WhoAmIOperation whoami = new WhoAmIOperation(response.getConnection());
